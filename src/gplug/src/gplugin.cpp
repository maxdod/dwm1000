...
void ActorPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  ros::init(argc, argv, "gazebo_client", ros::init_options::NoSigintHandler);
    ...
    this->client = this->nh->serviceClient<localizer_dwm1001::askPosition>("/getPosition");
...
void ActorPlugin::ChooseNewTarget()
{
 ...
 //std::cout << "chiedi nuovo target" << std::endl;
  
  
  if ((newTarget - this->target).Length() < 0.3)
  {
    ...
    if (this->client.call(srv))
  {
    ...
    if (this->xPos != srv.response.posX && this->yPos != srv.response.posY)
        {
          this->xPos = srv.response.posX;
          this->yPos = srv.response.posY;
          newTarget.X(this->xPos);
          newTarget.Y(this->yPos); 
        } else return;
