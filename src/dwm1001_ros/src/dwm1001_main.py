...
topic = str(rospy.get_param('~topicname'))
    ...
    def pubblishCoordinatesIntoTopics(self, networkDataArray):
        ...
        for network in networkDataArray:
            ...
            if 'AN' in network:
                ...
                anchor = Anchor( h, str(networkDataArray[networkDataArray.index(network) + 1]),
                ...
                pub_anchor = rospy.Publisher('/'+topic+'/anchor'+str(temp_anchor_number[-1]), Anchor, queue_size=1)
                ...
                # publish tag
                pub_anchor = rospy.Publisher('/'+ topic + '/tag', Tag, queue_size=1)
